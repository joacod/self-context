...
8. **Maintain navigation and provenance.** Update `learning/index.md`,
   ...
   the claim auditable.
9. **Use the shared backup and confirmation rules.** Create the provisional
   recovery backup before the authorized writes. Validate the result, create the
   project-root final backup, and discard the provisional only after final
   backup success. Batch at most one concise follow-up for selected high-impact,
