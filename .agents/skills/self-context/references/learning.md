...
8. **Maintain navigation and provenance.** Update `learning/index.md`,
   ...
   the claim auditable.
9. **Use the ordinary commit boundary for existing current vaults.** Prepare
   the semantic Learning page/source bytes, explicit activation decision, and
   log metadata; let the helper stage indexes and controls, validate, own both
   backups and rollback, and return one receipt. Missing or uninitialized
   bootstrap remains with [Initialization](initialization.md). Batch at most
   one concise follow-up for selected high-impact, ambiguous, contradictory, or
   inferred items. A source alone never verifies a Learning claim.
