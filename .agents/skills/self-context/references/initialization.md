...
the empty structure exists, complete the requested ingest or other mutation and
its validation, then create one normal post-write backup of the resulting state.
The same backup covers any first-use vertical activation in that operation.
Subsequent writes follow [the backup procedure](backups.md) after their
...
- **Schema 0.1:** preserve its text and legacy layout. A first meaningful
  ...
  before the normal post-write backup, but must not add contract markers or
...
- **Schema 0.2:** parse `vertical_contracts` strictly and treat it as
  ...
  `vertical@version`, adds its root link, completes the original operation, and
  then creates one normal post-write backup. It does not enable unrelated available
...
helper: the helper plans first, applies and validates the migration, owns the
single post-write backup, preserves pages and custom areas, and reports
...
- If a required control file is missing, create only the missing file after
  ...
  pages and links, validate the result, and create the post-write backup.
...
- schema 0.1: create only the required area, `index.md`, and root-index link;
  preserve the schema text and do not add a `vertical_contracts` entry, then
  validate and create the normal post-write backup;
- schema 0.2: create only the required area and `index.md`, add the exact
  available `vertical@version` entry, add its root-index link, and continue the
  original operation before validating and creating the post-write backup;
